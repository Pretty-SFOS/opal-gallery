...
Loader {
    ...
    function show(error, errorString) {
        var title = qsTranslate("Opal.MediaPlayer", "Playback failed", "error info page heading")
        var details = errorString + "\n\n"

        if (error === MediaPlayer.ResourceError) {
            title = qsTranslate("Opal.MediaPlayer", "Resource error", "error info page heading")
        } else if (error === MediaPlayer.FormatError) {
            qsTranslate("Opal.MediaPlayer", "Format error", "error info page heading")
        } else if (error === MediaPlayer.NetworkError) {
            qsTranslate("Opal.MediaPlayer", "Network error", "error info page heading")
        } else if (error === MediaPlayer.AccessDenied) {
            qsTranslate("Opal.MediaPlayer", "Access denied", "error info page heading")
        } else if (error === MediaPlayer.ServiceMissing) {
            qsTranslate("Opal.MediaPlayer", "Media service missing", "error info page heading")
        }
        ...
        if (error === MediaPlayer.ResourceError) {
            ...
            details += qsTranslate("Opal.MediaPlayer", "Playback failed due to an expected " +
    ...
    function showText(title, details, icon) {
        _title = title || _title || ""
